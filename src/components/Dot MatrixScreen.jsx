...
function DMD({
  ...
  const {
    ...
    borderPath,
  } = useBorderSequences({ cols: GRID_COLS, rows: GRID_ROWS });
    ...
    <section className={shellClassName}>
      <div className="dmd-screen" ref={screenRef}>
        ...
        {isScore ? (
          <div className="dmd-grid">
            <div className="dmd-layer">
              <DotMatrix sequence={scoreFrameSequence} {...borderConfig} />
            </div>
          </div>
        ) : null}
        {showFireworks ? (
          <div className="dmd-grid dmd-fireworks">
            <div className="dmd-layer">
              <DotMatrix sequence={fireworksSequence} {...fireworksConfig} />
            </div>
          </div>
        ) : null}
        {isGameOver ? (
          <div className="dmd-grid">
            <div className="dmd-layer">
              <DotMatrix sequence={gameOverBorderSequence} {...borderConfig} />
            </div>
          </div>
        ) : null}
        {shouldSnake ? (
          <div className="dmd-grid">
            {snakeFrames.map((frame, index) => (
              <div className="dmd-layer" key={`snake-${index}`}>
                <DotMatrix sequence={[frame]} {...snakeConfigs[index]} />
              </div>
            ))}
          </div>
        ) : null}
        {isJackpot ? (
          <div className="dmd-grid">
            <div className="dmd-layer">
              <DotMatrix sequence={borderSequence} {...borderConfig} />
            </div>
          </div>
        ) : null}
